...
ArUco mode:
  ...
  - Subscribes to /aruco_global_positions for ArUco marker detections
...
DEFAULT_ARUCO_TOPIC = "/aruco_global_positions"
...
class TestBackendNode(Node):
    ...
    def __init__(self):
        ...
        # ── Mission state ────────────────────────────────────────────────
        self.current_state = "IDLE"  # IDLE | GPS_NAV | SPIRAL_SEARCH | ARUCO_NAV | SPIRAL_COMPLETE_WAIT
        ...
        self.create_subscription(
            NavSatFix, "/converted_gps", self._gps_cb, self.sensor_qos
        )
        self.create_subscription(
            Float64,
    ...
    def _check_nav2_ready(self):
        ...
        elif not self._nav2_ready and self._nav2_check_count % 10 == 0:
    ...
    def _waypoint_cb(self, msg: String):
        ...
        if not self._nav2_ready:
            self.get_logger().warn("Nav2 not ready, cannot process waypoint")
            return
        ...
        # Convert GPS to odom-frame using INITIAL lat/lon/heading
    ...
    def _start_spiral_search(self):
        if self._spiral_center is None:
            self.get_logger().warn("No spiral center, cannot start spiral")
            return

        cx, cy = self._spiral_center
        self.spiral_points = spiral_search_points(cx, cy, self.spiral_radius)
        self.current_wp_index = 0
        self.current_state = "SPIRAL_SEARCH"

        self.get_logger().info(
            f"Spiral search: {len(self.spiral_points)} points "
    ...
    def _navigate_to_current_spiral_point(self):
        if self._is_stopped or self._is_paused:
            return

        if self.current_wp_index >= len(self.spiral_points):
            self.get_logger().info(
                "Spiral search complete. Waiting for ArUco detections..."
            )
            self.current_state = "SPIRAL_COMPLETE_WAIT"
            return

        x, y = self.spiral_points[self.current_wp_index]
        self.get_logger().info(
            f"Spiral {self.current_wp_index + 1}/{len(self.spiral_points)} "
            f"→ ({x:.2f}, {y:.2f})"
        )
        self._send_nav_goal(x, y, phase="spiral_segment")
        ...
        try:
            ...
            if updated > 0:
                ...
                if self.current_state in (
                    "GPS_NAV", "SPIRAL_SEARCH", "SPIRAL_COMPLETE_WAIT"
                ):
    ...
    def _select_unvisited_target(self) -> Optional[ArucoTarget]:
        ...
        if not fresh:
            return None
        return min(fresh, key=lambda t: math.hypot(t.x - rx, t.y - ry))
    ...
    def _interrupt_to_aruco(self, target: ArucoTarget):
        ...
        if self.current_goal_handle is not None:
            self.get_logger().info(
                f"ArUco {target.marker_id} detected — keeping current nav goal active"
            )
        else:
            self.get_logger().info(
                f"ArUco {target.marker_id} found — starting ArUco navigation"
            )
            self._start_aruco_navigation(target)

    def _start_aruco_navigation(self, target: ArucoTarget):
        if not self._nav2_ready:
            self.get_logger().warn("Nav2 not ready, cannot start ArUco navigation")
            return

        self._pending_aruco_target = target
        self.current_state = "ARUCO_NAV"

        # Instead of driving exact into the marker, calculate a goal 1.0m in front of it
        # This prevents depth accuracy loss at very close range and stops the rover from hitting the marker.
        ...
        #     stop_distance = 1.0  # Stop 1.0 meter away

        #     if dist > stop_distance:
        #         goal_x = target.x - (dx / dist) * stop_distance
        #         goal_y = target.y - (dy / dist) * stop_distance
        #         self.get_logger().info(f"ArUco {dist:.2f}m away. Halting {stop_distance}m in front.")
        #     else:
        #         self.get_logger().info(f"ArUco too close ({dist:.2f}m). Using exact pos.")
        
        self._send_nav_goal(goal_x, goal_y, phase="aruco_dock")
    ...
    def _tick(self):
        ...
        if self.current_state in ("SPIRAL_SEARCH", "SPIRAL_COMPLETE_WAIT"):
            target = self._select_unvisited_target()
            if target is not None:
                self._interrupt_to_aruco(target)
    ...
    def _goal_result_cb(self, future):
        ...
        # ── ArUco dock result ────────────────────────────────────────────
        elif phase == "aruco_dock":
            if status == GoalStatus.STATUS_SUCCEEDED:
                if self._pending_aruco_target is not None:
                    marker_id = self._pending_aruco_target.marker_id
                    if marker_id not in self.visited_markers:
                        self.visited_markers.append(marker_id)
                    self.get_logger().info(
                        f"✅ ArUco {marker_id} "
                        "reached!"
                    )
                ...
                self.get_logger().error(
                    f"ArUco dock failed (status={status_str})"
                )
                self._pending_aruco_target = None
                self.current_state = "IDLE"
                self._publish_failure(f"aruco_dock_failed_{status_str}")
